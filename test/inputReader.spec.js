...
describe("InputReader", () => {
	...
	it("should throw an error if the file doesn\'t have an extension", done => {
		inputReader.readFile(path.join(__dirname, 'data/sampleNoExtension'), err => {
			expect(err).to.exist;
			done();
		});
	});
	...
	it("should throw an error if the file is not a text file", done => {
		inputReader.readFile(path.join(__dirname, 'data/sampleInvalidFile.csv'), err => {
			expect(err).to.exist;
			done();
		});
	});
	...
	it("should read the contents of a text file correctly", done => {
		inputReader.readFile(path.join(__dirname, 'data/sampleData1.txt'), (err, fileData) => {
	...
	it("should throw an error if the file is empty", done => {
		inputReader.readFile(path.join(__dirname, 'data/sampleEmptyFile.txt'), err => {
			expect(err).to.exist;
			done();
		});
	});
	...
	it("should throw an error if the file doesn\'t exists", done => {
		inputReader.readFile(path.join(__dirname, 'data/fakeFile.txt'), err => {
			expect(err).to.exist;
			done();
		});
	});
